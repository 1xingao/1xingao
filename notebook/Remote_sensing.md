...
2. 对像面扫描成像仪 ： 瞬间在像面上形成一幅影响，然后对影像进行扫描
3. 成像扫描仪
...
### 1. 中心投影构象方程：
   ...
   **构象方程表示的是物点和像点的几何关系**![中心构像方程](https://fastly.jsdelivr.net/gh/1xingao/picgo@main/img/16661775910571666177590875.png)
   ...
   s为传感器中心投影，p在地面的坐标，A为传感器坐标系相对地面的旋转矩阵
...
遥感图像的分类一句的是数字图像中反应的同类地物的光谱相似性和异类地物的光谱差异性
...
1. 混淆矩阵
   ...
   - 生产者精度 = 每一类中正确分类的像元数（位于主对角线上） 除以 参考数据中的该类的像元总数（除以行和）
2. kappa系数
